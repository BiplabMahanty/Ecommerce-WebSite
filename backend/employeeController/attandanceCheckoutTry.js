...
function verifyFaceMatch(storedDescriptor, currentDescriptor, threshold = 0.6) {
    ...
    try {
        ...
        if (!existing) {
            ...
            if (existRockstar) {
                ...
                if (!existPrevious) {
                    // Check if leave & allLeaveDays exist and is an array
                    const leaveDays = Array.isArray(leave?.allLeaveDays) ? leave.allLeaveDays : [];

                    // If today is in leave days → no absent increment
                    const isOnLeave = leaveDays.some(d =>
